      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-8 w-full bg-gray-50 rounded-lg p-6 flex flex-col md:flex-row md:items-start md:gap-8 shadow-inner">
          ...
          <div className="flex-1 md:border-l md:pl-8 border-gray-200">
            <div className="mb-2">
              <span className="font-semibold text-lg text-gray-800">Promedio:</span> <span className="text-yellow-500 font-bold">{product.averageRating} / 5</span>
            </div>
            <div>
              <span className="font-semibold text-lg text-gray-800">Reseñas:</span>
              <ul className="mt-2 space-y-2">
                {product.reviews.map((review) => (
                  <li key={review.id} className="bg-white rounded p-2 shadow-sm border border-gray-100">
                    <span className="font-medium text-gray-900">{review.userName}:</span> {review.comment} <span className="text-yellow-500">({review.rating}★)</span>
                  </li>
                ))}
              </ul>
            </div>
